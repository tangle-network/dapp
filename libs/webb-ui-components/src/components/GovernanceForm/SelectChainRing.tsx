...
import { getChainIconClassNameByIdx } from './utils';

const SelectChainRing: FC<{
  typedChainIds: number[];
  currentSelectedTypedChainId?: number;
  handleSelectedChain: (typedChainId: number) => void;
}> = ({ typedChainIds, currentSelectedTypedChainId, handleSelectedChain }) => {
  const [isDarkMode] = useDarkMode();

  const getStrokeColor = useCallback(
    (idx: number) => {
      if (currentSelectedTypedChainId === undefined) return '#9CA0B0';
      return currentSelectedTypedChainId === typedChainIds[idx]
        ? isDarkMode
          ? '#4B3AA4'
          : '#B5A9F2'
        : '#9CA0B0';
    },
    [currentSelectedTypedChainId, typedChainIds, isDarkMode]
  );

  return (
    <div className="relative w-fit">
      {typedChainIds.map((typedChainId, idx) => {
      ...
      >
        <div>
          ...
          >
            {currentSelectedTypedChainId !== undefined
              ? chainsConfig[currentSelectedTypedChainId].name
              : 'Select Chain'}
          </Typography>
        </div>
      </div>
...
export default SelectChainRing;
