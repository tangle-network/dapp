...
import { StakingAsset, StakingAssetMetadata } from '../../types/staking';
...
import useStakingAssetBalances from './useStakingAssetBalances';
import { useVaultAssets } from './useVaultAssets';
import useBalances from '../../hooks/useBalances';

const useSubstrateStakingAssets = () => {
  ...
  const assetIds = useMemo(() => {
    ...
    const assetIdsSet = new Set<StakingAssetId>();
        ...
        return api.query.assets.asset.entries().pipe(
          map((entries) => {
            const keyValues = entries.flatMap(([key, detailsOpt]) => {
              ...
              const assetId = assertStakingAssetId(
      ...
      return api.query.rewards.assetLookupRewardVaults.entries().pipe(
        map((entries) => {
          const keyValues = entries.flatMap(([key, vaultIdOpt]) => {
            ...
            const assetId = createStakingAssetId(key.args[0]);
  ...
  const nativeAssets = useMemo<StakingAssetMetadata[] | null>(() => {
  ...
  const assetMap = useMemo(() => {
    ...
    const map = new Map<StakingAssetId, StakingAssetMetadata>();
  ...
  const assetsWithBalances = useMemo(() => {
    ...
    const map = new Map<StakingAssetId, StakingAsset>();
...
export default useSubstrateStakingAssets;
