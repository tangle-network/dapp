...
import useStakingRewardConfig from '../../hooks/useStakingRewardConfig';
...
type UseStakingVaultsOptions = {
  assets: Map<StakingAssetId, StakingAsset> | null;
  delegatorInfo: DelegatorInfo | null;
  assetsTvl: Map<StakingAssetId, BN> | null;
...
export const useStakingVaults = ({
  ...
  delegatorInfo,
  assetsTvl,
}: UseStakingVaultsOptions) => {
  const rewardConfig = useStakingRewardConfig();
...
type Options = {
  ...
  networkId?: NetworkId;
  delegatorInfo?: DelegatorInfo | null;
  assetTvl?: Map<StakingAssetId, BN> | null;
...
export const createVaultMap = ({
  ...
  rewardConfig,
  networkId,
}: Options): Map<number, StakingVault> => {
  const vaults = new Map<number, StakingVault>();
...
const createTntVault = ({
...
}: CreateTntVaultOptions): StakingVault => {
  const NATIVE_ASSET_ID = '0' as StakingAssetId;
  ...
  const tntMetadata: StakingAssetMetadata = {
