...
import useStakingAssetIds from './useStakingAssetIds';

const useStakingAssetBalances = () => {
  const substrateAddress = useSubstrateAddress();
  const { assetIds, isLoading: isLoadingAssetIds } = useStakingAssetIds();
  ...
  const balances = useMemo(() => {
    const balances = new Map<StakingAssetId, BN>();
...
export default useStakingAssetBalances;
