import { Transaction, NewNotesTxResult } from '@webb-tools/abstract-api-provider';
...
export class PolkadotVAnchorActions extends VAnchorActions<WebbPolkadot> {
  fetchSmallFixtures(tx: Transaction<NewNotesTxResult>, maxEdges: number): Promise<ZkComponents> {
    throw new Error('Method not implemented.');
  }
  fetchLargeFixtures(tx: Transaction<NewNotesTxResult>, maxEdges: number): Promise<ZkComponents> {
    throw new Error('Method not implemented.');
  }
  transact(
