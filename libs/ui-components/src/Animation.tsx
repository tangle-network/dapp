...
export const TextAnimation: FC<{ value: ReactNode } & BareProps> = ({ className, value }) => {
  ...
  return (
    <TextAnimationRoot className={className} ref={$container}>
      <Motion
        defaultStyle={{
          opacity: 1,
          top: 0,
      ...
      >
        {(style): JSX.Element => {
          return (
            <TextAnimationItem opacity={style.opacity} top={style.top}>
              {prevValue}
            </TextAnimationItem>
          );
        }}
      ...
      >
        {(style): JSX.Element => (
          <TextAnimationItem opacity={style.opacity} ref={$value} top={style.top}>
            {value}
          </TextAnimationItem>
        )}
      </Motion>
