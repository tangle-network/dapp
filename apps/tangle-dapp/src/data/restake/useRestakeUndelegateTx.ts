...
const useRestakeUndelegateTx = () => {
  const { assets } = useRestakeAssets();

  const evmTxFactory: EvmTxFactory<
    typeof RESTAKING_PRECOMPILE_ABI,
    'scheduleDelegatorUnstake' | 'scheduleDelegatorNominationUnstake',
    Context
  > = useCallback(
    (context) => ({
  ...
  const substrateTxFactory: SubstrateTxFactory<Context> = useCallback(
    (api, _activeSubstrateAddress, context) => {
      if (context.isNominatedAsset) {
        return api.tx.multiAssetDelegation.scheduleNominationUnstake(
  ...
  const getSuccessMessage: GetSuccessMessageFn<Context> = useCallback(
    ({ assetId, amount }) => {
      ...
      return `Scheduled unstake of ${formattedAmount} ${symbol}.`;
    },
    [assets],
  );
...
export default useRestakeUndelegateTx;
