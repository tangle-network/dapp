...
const useRestakeCancelWithdrawRequestsTx = () => {
  const evmTxFactory: EvmTxFactory<
    typeof BATCH_PRECOMPILE_ABI,
    'batchAll',
    Context
  > = useCallback((context) => {
    const batchCalls = context.withdrawRequests.map(({ assetId, amount }) => {
  ...
  const substrateTxFactory: SubstrateTxFactory<Context> = useCallback(
    (api, _activeSubstrateAddress, context) => {
      const txs = context.withdrawRequests.map(({ assetId, amount }) => {
...
export default useRestakeCancelWithdrawRequestsTx;
