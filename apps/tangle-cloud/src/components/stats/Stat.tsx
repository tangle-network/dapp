  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        {icon ? (
          <span className="shrink-0 text-mono-100 dark:text-mono-60 [&_svg]:h-3 [&_svg]:w-3">
            {icon}
          </span>
        ) : null}
        <p
          className={twMerge(
            'truncate font-medium text-mono-100 dark:text-mono-60 uppercase tracking-wider',
            sizing.label,
          )}
        >
          {label}
      ...
      {sublabel ? (
        <p className="mt-1 truncate text-mono-100 dark:text-mono-60 text-xs">
          {sublabel}
        </p>
      ) : null}
    </div>
