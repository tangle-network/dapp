      ...
      (apiRx) => {
        ...
        const runningServices$ =
          apiRx.query.services?.instances === undefined
            ...
            : apiRx.query.services?.instances
                ...
                .pipe(
                  map((instances) => {
                    const runningServices = instances.filter(
                      ([_, instance]) => {
                        if (instance.isNone) {
                          return false;
                        }
                        ...
                        try {
                          ...
                          const normalizedUser = accountAddress
                            ? encodeAddress(decodeAddress(accountAddress))
                            : null;

                          return normalizedOwner === normalizedUser;
                        } catch (error) {
                          console.error(
                            'Address normalization error in useUserStatsData:',
                            error,
                          );
        ...
        const deployedServices$ =
          apiRx.query.services?.instances === undefined
            ...
            : apiRx.query.services?.instances
                ...
                .pipe(
                  map((instances) => {
                    const deployedServices = instances.filter(
                      ([_, instance]) => {
                        if (instance.isNone) {
                          return false;
                        }
        ...
        const pendingServices$ =
          apiRx.query.services?.serviceRequests === undefined
            ? of({})
            : apiRx.query.services?.serviceRequests
                .entries<
                  Option<TanglePrimitivesServicesServiceServiceRequest>
                >()
                .pipe(
                  map((serviceRequests) => {
        ...
        const consumedServices$ =
          apiRx.query.services?.instances === undefined
            ...
            : apiRx.query.services?.instances
                ...
                .pipe(
                  map((instances) => {
                    const consumedServices = instances.filter(
                      ([_, instance]) => {
                        if (instance.isNone) {
                          return false;
                        }
