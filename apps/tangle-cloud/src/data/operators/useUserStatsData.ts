...
export const useUserStatsData = (
  ...
  const { result: userStats, ...rest } = useApiRx(
    useCallback((apiRx) => {
      ...
      const pendingServices$ = apiRx.query.services?.serviceRequests === undefined
        ? of({})
        : apiRx.query.services?.serviceRequests.entries().pipe(
            map((serviceRequests) => {
              const pendingServices = serviceRequests.filter(([_, serviceRequest]) => {
      ...
      const consumedServices$ = apiRx.query.services?.instances === undefined
        ? of({})
        : apiRx.query.services?.instances.entries().pipe(
            map((instances) => {
              const consumedServices = instances.filter(([_, instance]) => {
                ...
                const detailed = instance.unwrap();
                return detailed.permittedCallers.some((caller) => caller.toHuman() === accountAddress);
              });
              return {
      ...
      return combineLatest([
        ...
        .pipe(
          map(([runningServices, deployedServices, pendingServices, consumedServices]) => {
            return {
              ...runningServices,
              ...deployedServices,
              ...pendingServices,
