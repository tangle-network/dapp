...
import { useCancelExitTx } from './useCancelExitTx';
...
describe('useCancelExitTx', () => {
  ...
  it('uses cancelExit with service id args', async () => {
    let capturedFactory:
    ...
    expect(config.functionName).toBe('cancelExit');
    expect(config.args).toEqual([15n]);
  });
  ...
  it('invalidates exit scheduling queries after successful tx', () => {
    ...
    renderHook(() => useCancelExitTx({ onSuccess }));
