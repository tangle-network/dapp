...
import { useExecuteExitTx } from './useExecuteExitTx';
...
describe('useExecuteExitTx', () => {
  ...
  it('uses executeExit with service id args', async () => {
    let capturedFactory:
    ...
    expect(config.functionName).toBe('executeExit');
    expect(config.args).toEqual([14n]);
  });
  ...
  it('invalidates exit and service queries after successful tx', () => {
    ...
    renderHook(() => useExecuteExitTx({ onSuccess }));
