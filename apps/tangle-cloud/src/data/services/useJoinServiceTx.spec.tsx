...
import { useJoinServiceTx } from './useJoinServiceTx';
...
describe('useJoinServiceTx', () => {
  ...
  it('uses joinService with service id and exposure args', async () => {
    let capturedFactory:
    ...
    expect(config.functionName).toBe('joinService');
    expect(config.args).toEqual([10n, 8750]);
  });
  ...
  it('invalidates service and exit queries after successful tx', () => {
    ...
    renderHook(() => useJoinServiceTx({ onSuccess }));
