...
const getNewProposal = (): ProposalListItem => {
  ...
  return {
    ...
    status: Object.keys(ProposalStatus)[
      Math.floor(Math.random() * 7)
    ] as ProposalStatus,
    type: Object.keys(ProposalType)[
      Math.floor(Math.random() * 16)
    ] as ProposalType,
    txHash: randEthereumAddress(),
    proposers: {
      firstElements: arrayFrom(rand, () =>
        randHexaDecimal({ length: 64 }).join('')
      ),
      count: rand,
    },
 ...
 */
export const fetchProposalsData = async (options: {
  pageIndex: number;
  pageSize: number;
}) => {
