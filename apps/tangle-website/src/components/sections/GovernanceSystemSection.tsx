...
export const GovernanceSystemSection = () => {
  return (
    <section className="dark bg-mono-200">
      <div className="mx-auto lg:w-3/4 flex flex-col lg:flex-row-reverse lg:gap-6 pt-[40px] pb-[80px] lg:py-[96px]">
        <div className="lg:mr-[-25vh] lg:flex-1 flex items-center">
        ...
        <div className="lg:flex-1 px-5">
          ...
          <div className="mt-[56px] flex flex-col gap-6">
            {governanceSystemQnAItems.map((item, index) => (
              <GovernanceQnA
                key={index}
                Icon={item.icon}
                title={item.title}
...
const GovernanceQnA = ({ Icon, title, description, href }) => {
  return (
    <div>
      <Icon />
      <Typography variant="h4" className="font-bold text-mono-0 mt-6 mb-4">
        {title}
      </Typography>
