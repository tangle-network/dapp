...
const StepCards: FC = () => {
  ...
  const getColumnKeyAsTitle = (key: ColumnKey): string => {
    switch (key) {
  ...
  const columns: ColumnDef<RowData, any>[] = [
    columnHelper.accessor(ColumnKey.IsChecked, {
      ...
      cell: (props) => <CheckBox isChecked={true} />,
    }),
    ...
    columnHelper.accessor(ColumnKey.Uptime, {
      ...
      cell: (props) => (
        <Progress suffixLabel="%" className="max-h-7" size="lg" value={99.3} />
      ),
    }),
  ];
  ...
  return (
    <div className="flex flex-col gap-6 flex-grow">
      ...
      <StepCard title="Select Service Tier" number={5} onNext={handleNextStep}>
...
const StepCard: FC<StepCardProps> = ({ number, title, children, onNext }) => {
  return (
    <CollapsibleCard title={`${number}. ${title}:`}>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-4">
          ...
          <Button isFullWidth onClick={onNext} variant="primary">
            Next
          </Button>
        </div>
      </div>
...
const ServiceTierCard: FC<ServiceTierCardProps> = ({
  ...
  return (
    <Card className="flex flex-col gap-1 space-y-0 rounded-2xl w-full dark:bg-mono-160">
      ...
      {monthlyPrice !== undefined && (
        <>
          ...
          <Typography variant="body1" fw="normal"></Typography>
        </>
      )}
      ...
      >
        {monthlyPrice === null ? 'Contact Us' : 'Select Plan'}
      </Button>
    </Card>
  );
};
