    ...
    (value: number): Promise<any> => {
      ...
      if (_config.max && value > _config.max.value) {
        return Promise.reject(
          new Error(
            _config.max.customText ? _config.max.customText : `Greater than the max amount ${_config.max.value}`
          )
        );
      }
      ...
      if (_config.min && value < _config.min.value) {
        return Promise.reject(
          new Error(_config.min.customText ? _config.min.customText : `Less than The min amount ${_config.min.value}`)
        );
      }
  ...
  const fn = useCallback(
    ({ amount }: Partial<BalanceInputValue>): Promise<any> => {
      ...
      if (_config.max && _amount.isGreaterThan(_config.max[0])) {
        return Promise.reject(
          new Error(_config.max[1] ? _config.max[1] : `Greater than the max amount ${_config.max[0].toNumber()}`)
        );
      }

      if (_config.min && _amount.isLessThan(_config.min[0])) {
        return Promise.reject(
          new Error(_config.min[1] ? _config.min[1] : `Less than The min amount ${_config.min[0].toNumber()}`)
        );
      }
