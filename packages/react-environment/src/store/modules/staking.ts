...
export const subscribeStakingPool = (api: ApiRx): Observable<StakingPoolData> => {
  ...
  return stakingPool$.pipe(
    map(
      (data: DerivedStakingPool): StakingPoolData => {
        const stakingPool = new StakingPool({
          ...
          stakingPoolParams: {
            ...
            targetMaxFreeUnbondedRatio: FixedPointNumber.fromInner(
              data.stakingPoolParams.targetMaxFreeUnbondedRatio.toString()
            ),
            targetMinFreeUnbondedRatio: FixedPointNumber.fromInner(
              data.stakingPoolParams.targetMinFreeUnbondedRatio.toString()
            ),
            targetUnbondingToFreeRatio: FixedPointNumber.fromInner(
              data.stakingPoolParams.targetUnbondingToFreeRatio.toString()
            ),
          },
