...
export const Dropdown: FC<Props> = memo(
  ({
    ...
    const active = config.find((data: DropdownConfig) =>
      compareFN ? compareFN(data.value, value) : data.value === value
    );
    ...
    return (
      ...
      >
        <div className={classes.activeRoot} onClick={toggleMenu}>
          <div className={clsx(classes.activeContent, activeContentClassName)}>{renderSelected()}</div>
