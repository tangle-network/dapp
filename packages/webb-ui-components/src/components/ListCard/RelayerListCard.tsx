import { ExternalLinkLine, Search } from '@webb-dapp/webb-ui-components/icons';
...
export const RelayerListCard = forwardRef<HTMLDivElement, RelayerListCardProps>(
  ({ isDisconnected, onChange, onClose, onConnectWallet, relayers, value: selectedRelayer, ...props }, ref) => {
    const [, setRelayer] = useState<RelayerType | undefined>(() => selectedRelayer);
    ...
    return (
      <ListCardWrapper {...props} title={`Select a relayer`} onClose={onClose} ref={ref}>
        {/** The search input */}
              ...
              return (
                ...
                >
                  <div className='flex items-center space-x-2'>
                    ...
                    <a href={current.externalUrl} rel='noreferrer noopener' target='_blank'>
                      <ExternalLinkLine />
                    </a>
                  </div>
                  ...
                  <div className='flex items-center space-x-2'>
                    {current.fee && (
                      <Typography variant='utility' className='uppercase text-mono-100 dark:text-mono-80'>
                        Fee: {getRoundedAmountString(parseFloat(current.fee.toString()))}
                      </Typography>
                    )}

                    {current.percentage && (
                      <Typography variant='utility' className='uppercase text-mono-140 dark:text-mono-0'>
        ...
        >
          ...
          <Button variant='link' size='sm' className='mt-1 text-center' onClick={onConnectWallet}>
