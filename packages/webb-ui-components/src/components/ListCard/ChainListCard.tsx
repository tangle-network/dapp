import { Search, TokenIcon } from '@webb-dapp/webb-ui-components/icons';
...
export const ChainListCard = forwardRef<HTMLDivElement, ChainListCardProps>(
  ({ chainType, chains, onChange, onClose, value: selectedChain, ...props }, ref) => {
    ...
    return (
      <ListCardWrapper
        {...props}
        title={`Select ${chainType === 'source' ? 'Source' : 'Destination'} Chain`}
        onClose={onClose}
        ref={ref}
      >
        {/** The search input */}
        ...
        <ScrollArea className='min-w-[350px] h-[376px]'>
          <ul className='p-2'>
            {filteredChains.map((currentChain, idx) => {
              const isSelected = currentChain.name === chain?.name && currentChain.symbol === chain?.symbol;
              ...
              return (
                ...
                >
                  <div className='flex items-center space-x-2'>
                    <TokenIcon isActive={isSelected} size='lg' name={currentChain.symbol.toLowerCase()} />
